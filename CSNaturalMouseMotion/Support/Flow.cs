namespace Zaac.CSNaturalMouseMotion.Support
{
    ...
    public class Flow
    {
        private const int AVERAGE_BUCKET_VALUE = 100;

        private readonly double[] buckets;

        /// <param name="characteristics"> the characteristics array, which can be any size, contain non-negative numbers.
        ...
        /// than it was in the first 25% of the time. </param>
        ...
        private double[] NormalizeBuckets(double[] flowCharacteristics)
        {
            ...
            if (sum == 0)
            {
                throw new System.ArgumentException("Invalid FlowCharacteristics. All array elements can't be 0.");
            }
            /// <summary>
        ...
        public virtual double GetStepSize(double distance, int steps, double completion)
        {
        ...
        /// returns 0.4 * bucket[0] + 1 * bucket[1] + 0.4 * bucket[2] </summary>
